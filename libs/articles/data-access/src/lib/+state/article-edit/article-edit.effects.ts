  ...
  (
    ...
    return actions$.pipe(
      ...
      concatMap(([_, data]) => {
        ...
        if (typeof processedData.tagList === 'string') {
          processedData.tagList = processedData.tagList.split(',').map((tag: string) => tag.trim());
        }
      
        return articlesService.publishArticle(processedData).pipe(
